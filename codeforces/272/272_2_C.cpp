...
template <typename T> T modSum( T u, T v)
{
	u=u%MOD;
	v=v%MOD;
	return (u + v)%MOD;
...
template <typename T> T modSub( T u, T v)
{
	u=u%MOD;
	v=v%MOD;
	return (u - v)%MOD;
...
template<typename T> T modMult(T u, T v)
{
	u=u%MOD;
	v=v%MOD;
	return (u * v)%MOD;
...
int main()
{
	...
	{
		x=(b/2);
		first=modMult(modMult(modMult(modMult(x,x),a),b-1),a+1);
		second=modMult(modMult(x,a),(b-1));
	}
	...
	else if(b%2==1 && a%2==0)
	{
		x=(b-1)/2;
		y=a/2;
		first=modMult(modMult(modMult(modMult(x,b),b),y),a+1);
		second=modMult(modMult(a,b),x);
	}
	...
	else
	{
		x=(b-1)/2;
		y=(a+1)/2;
		first=modMult(modMult(modMult(modMult(a,b),b),x),y);
		second=modMult(modMult(a,b),x);
	}
	sum=modSum(first,second);
	cout << sum;
