...
int main()
{
	int n,m,k;
	cin >> n >> m >> k;
	int right=(n-k), left=(k-1);
	int ans=2,leftitr=1,rightitr=1;
	if(m==n)
	{
		cout << "1" << "\n";
		return 0;
	...
	while(m>0)
	{
		if(min2(leftitr,left)==left && min2(rightitr,right)==right)
		{
			cout << ans+(m/(left+right+1)) << "\n";
			return 0;
		}
		m=m-min2(leftitr,left);
		m=m-min2(rightitr,right);
		leftitr++,rightitr++;
		if(m>0)
		{
			ans++;
