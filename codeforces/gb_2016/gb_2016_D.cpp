...
void DFS(int branch, int x, int y, pair< pair<int,int>, pair<int,int> > newDir)
{
	...
	memory[x+200][y+200][branch-1][fDir.F+1][fDir.S+1]=1;
	memory[x+200][y+200][branch-1][sDir.F+1][sDir.S+1]=1;
	fab(i,1,t[branch-1])
	{
		newX=(x+(fDir.F*i));
		newY=(y+(fDir.S*i));
	...
	if(memory[newX+200][newY+200][branch][newFDir.F.F+1][newFDir.F.S+1]==0)
	{
		DFS(branch+1,newX,newY,newFDir);	
	}

	newX=x+(sDir.F*t[branch-1]),newY=y+(sDir.S*t[branch-1]);
	if(memory[newX+200][newY+200][branch][newSDir.S.F+1][newSDir.S.S+1]==0)
	{
