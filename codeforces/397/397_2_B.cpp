...
int main()
{
	...
	int init_val=-1,char_val,check_val=49;
	if(s[0]!='a')
	{
		debug("NO");
		return 0;
	...
	for(auto it: M)
	{
		//debug2(it.S,init_val);
		char_val=it.F-'0';
		//debug2(char_val,check_val);
		if(!((char_val==check_val) ||  (char_val==(check_val+1))))
		{
			debug("NO");
			return 0;
		}
		else if(char_val==(check_val+1))
		{
			check_val++;
		}
